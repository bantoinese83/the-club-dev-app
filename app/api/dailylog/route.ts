  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        streak: {
          increment:
            user.lastLogDate && new Date(user.lastLogDate) >= today ? 0 : 1,
        },
        lastLogDate: today,
      },
    });
    ...
    return NextResponse.json(
      { success: false, error: 'An error occurred while saving the daily log' },
      { status: 500 },
    );
  }
}
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'An error occurred while fetching daily logs' },
      { status: 500 },
    );
  }
}
