...
export function UnsplashImage() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      ...
      if (cachedImage) {
        const { url, timestamp, photographerName, photographerUsername }: CachedImage = JSON.parse(cachedImage)
        if (Date.now() - timestamp < CACHE_DURATION) {
          setImageUrl(url)
          setPhotographer({ name: photographerName, username: photographerUsername })
          return
        }
      }

      try {
        const response = await fetch('https://api.unsplash.com/photos/random?query=workspace&orientation=portrait', {
          headers: {
