...
export default function GitHubIntegration({ setIsLoading }: GitHubIntegrationProps) {
  ...
  useEffect(() => {
    setIsLoading(isLoading || isAiLoading)
  }, [isLoading, isAiLoading, setIsLoading])
                ...
                {commits.map((commit) => (
                  <div key={commit.id} className="flex items-start space-x-2 mb-2">
                    ...
                    <div>
                      ...
                      <Button variant="outline" size="sm" className="mt-1" onClick={() => handleCodeReview(commit.id)}>
